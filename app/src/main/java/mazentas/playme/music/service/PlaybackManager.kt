...
class PlaybackManager(val context: Context) {
    ...
    fun maybeSwitchToCrossFade(crossFadeDuration: Int): Boolean {
        /* Switch to SmartExoPlayer if CrossFade duration is 0 and
                Playback is not an instance of SmartExoPlayer */
        if (playback !is SmartExoPlayer && crossFadeDuration == 0) {
            if (playback != null) {
                playback?.release()
            }
            playback = null
            playback = SmartExoPlayer(context)
            return true
    ...
    private fun createLocalPlayback(): Playback {
        // Set SmartExoPlayer when crossfade duration is 0 i.e. off
        return if (PreferenceUtil.crossFadeDuration == 0) {
            SmartExoPlayer(context)
        } else {
